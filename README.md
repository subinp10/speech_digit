...
The resulting sequence becomes the observation sequence for the HMM.
...
An HMM can represent the temporal evolution of a spoken digit through a sequence of hidden states.
...
1. Computes forward probabilities.
2. Computes backward probabilities.
3. Estimates state occupation probabilities.
4. Estimates state transition probabilities.
5. Updates observation probabilities.
...
The model parameters are then re-estimated using these probabilities.
...
Some important parameters used in the system are:
...
| Frame size          |  320 samples |
| Frame shift         |        10 ms |
| Training utterances | 30 per digit |
| Testing utterances  | 10 per digit |
...
\hat{d}
=
\arg\max_d P(O|\lambda_d)
$$

where \(\lambda_d\) is the HMM corresponding to digit \(d\).
...
The project demonstrates an end-to-end traditional speech recognition pipeline:
...
It provides practical understanding of how **classical speech recognition systems** can be designed using signal processing, probability, and statistical sequence modeling techniques.
